...
def parse_month_sheet(ws,month):
    # 月の収入は各月シートのB列に記載された金額を合計する。B7は残高セルのため除外。
    income=0
    ...
    for row_no in range(1,ws.max_row+1):
        ...
        payments.append({"name":str(detail),"type":category,"amount":amount,"progress":list(ratio) if ratio else None,"row":row_no})
    balance=ws["B7"].value
...
def extract_split_items(ws):
    ...
    for row_no in range(3,ws.max_row+1):
        ...
        items.append({"name":str(name),"type":"リボ" if str(name).strip().upper()=="R" else "分割","total":total if number(total) else None,"monthly":monthly,"end":str(end) if end is not None else None,"remainingAmount":remaining if number(remaining) else None,"row":row_no})
    return items
