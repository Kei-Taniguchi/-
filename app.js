...
function plannedAmount(p,monthsFromLatest){if(p.type==='分割'&&p.baseCount!=null)return monthsFromLatest<p.baseCount?p.monthly:0;return p.monthly}
...
function render(){
  ...
  const forecast=[];
  for(let i=0;i<12;i++){
    const m=addMonths(latest?.month||new Date().toISOString().slice(0,7),i), income=latest?.income||0;
    const fixed=data.payments.filter(p=>p.type==='固定費').reduce((s,p)=>s+plannedAmount(p,i),0);
    const utilities=data.payments.filter(p=>['電気','ガス','水道'].includes(p.type)).reduce((s,p)=>s+plannedAmount(p,i),0);
    const installments=data.payments.filter(p=>['リボ','分割'].includes(p.type)).reduce((s,p)=>s+plannedAmount(p,i),0);
    const other=data.payments.filter(p=>p.type==='その他').reduce((s,p)=>s+plannedAmount(p,i),0);
    const total=fixed+utilities+installments+other;
    forecast.push({month:m,income,fixed,utilities,installments,other,total,balance:income-total});
  }
  ...
  document.querySelector('#months').innerHTML=allRows.map((m,idx)=>{const actual=idx<data.months.length;const remain=m.balance==null?m.income-m.total:m.balance;return `<tr><td><strong>${monthLabel(m.month)}</strong>${actual?'':'<small>（予測）</small>'}</td><td>${yen(m.income)}</td><td>${yen(m.fixed)}</td><td>${yen(m.utilities)}</td><td>${yen(m.installments)}</td><td>${yen(m.other)}</td><td><strong>${yen(m.total)}</strong></td><td class="${remain>=0?'positive':'negative'}">${yen(remain)}</td></tr>`}).join('');
